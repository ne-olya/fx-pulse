...
/* ---------- 1. Проблема ---------- */
{
  ...
  txt(s,"Домой, семье, раз в месяц\nпосле расчёта. День\nвыбирает наугад",7.08,1.9,5.45,3,30,"3A3A3A");
  ...
  txt(s,"разница между лучшим и худшим днём",4.5,4.03,8.2,1,30,INK,true);
  txt(s,"месяца на переводе 45 000 ₽",4.5,4.53,8.2,1,30,"3A3A3A");
  ...
  txt(s,"Момент за клиента не выбирает никто",0.85,6.47,11.6,1,26,INK,true);
}

/* ---------- 2. Сравнение коридоров ---------- */
{
  const s=slide("Какой коридор","и почему");
  const head=["Коридор","Uplift","Сигналов","Клиенту","Банку"];
  const rows=[["Узбекистан","1,56","0,61","1 053 ₽","40,5 млн"],
              ["Армения","1,44","1,11","450 ₽","31,5 млн"],
              ["Таджикистан","1,44","1,14","416 ₽","18,0 млн"],
              ["Киргизия","1,38","1,11","341 ₽","18,0 млн"],
              ["Казахстан","1,30","1,11","153 ₽","9,0 млн"]];
  ...
  txt(s,"Узбекистан выигрывает и по модели, и по деньгам банка",0.85,5.18,11.6,1,26,INK,true);
  txt(s,"Клиенту — при 9 переводах в год по 45 000 ₽",0.85,5.66,11.6,1,26,"3A3A3A");
  txt(s,"Банку — на 100 тыс. клиентов, маржа 2 %, рост переводов 5 %",0.85,6.12,11.6,1,26,"3A3A3A");
}
...
/* ---------- 3. Как устроено ---------- */
{
  ...
  txt(s,"Что мы про решение знаем",0.8,4.28,7.45,1,30,INK,true);
  txt(s,"Строгая мера uplift — 1,25\n\nБерём 26 пунктов из 129\n\nКурс ЦБ — не курс сделки",0.8,4.9,7.45,5,24,"3A3A3A");
...
/* ---------- 4. Готовность и пилот ---------- */
{
  ...
  txt(s,"Тест на 20 000 в группе различает прирост от 2,9 %.",0.85,5.0,11.6,1,26,"3A3A3A");
  txt(s,"Меньше — не отличим от шума, и это не успех",0.85,5.46,11.6,1,26,"3A3A3A");
