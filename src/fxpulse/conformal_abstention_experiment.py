"""Rolling conformal singleton-positive policy with delayed outcome availability."""

from __future__ import annotations

import argparse
import hashlib
import json
from pathlib import Path
from typing import Any

import numpy as np
import pandas as pd

from fxpulse.adaptive_threshold import adaptive_candidates
from fxpulse.next_hypotheses import _apply_policy
from fxpulse.temporal_sequence_experiment import summarize


def load_config(path: Path | str = Path("configs/conformal_abstention_experiment.json")) -> dict[str, Any]:
    config = json.loads(Path(path).read_text(encoding="utf-8"))
    if config.get("schema_version") != 1 or config.get("status") != "registered_before_results":
        raise ValueError("conformal config must be preregistered")
    return config


def conformal_positive_candidates(
    frame: pd.DataFrame,
    *,
    horizon: int,
    alpha: float,
    lookback: int,
    minimum_history: int,
) -> pd.DataFrame:
    ordered = frame.sort_values("timestamp", kind="mergesort").copy()
    scores = ordered["score"].to_numpy(dtype=float)
    targets = ordered["target"].to_numpy(dtype=int)
    p_positive = np.full(len(ordered), np.nan)
    p_negative = np.full(len(ordered), np.nan)
    for index, score in enumerate(scores):
        observable_end = max(0, index - int(horizon) + 1)
        calibration_start = max(0, observable_end - int(lookback))
        calibration_scores = scores[calibration_start:observable_end]
        calibration_targets = targets[calibration_start:observable_end]
        valid = np.isfinite(calibration_scores)
        if valid.sum() < int(minimum_history) or not np.isfinite(score):
            continue
        probabilities_true = np.where(
            calibration_targets[valid] == 1,
            calibration_scores[valid],
            1 - calibration_scores[valid],
        )
        nonconformity = 1 - probabilities_true
        p_positive[index] = (1 + np.sum(nonconformity >= 1 - score)) / (len(nonconformity) + 1)
        p_negative[index] = (1 + np.sum(nonconformity >= score)) / (len(nonconformity) + 1)
    ordered["conformal_p_positive"] = p_positive
    ordered["conformal_p_negative"] = p_negative
    selected = ordered.loc[(ordered["conformal_p_positive"] > alpha) & (ordered["conformal_p_negative"] <= alpha)].copy()
    selected["score_threshold"] = 1 - alpha
    return selected


def evaluate(scores: pd.DataFrame, config: dict[str, Any]) -> tuple[pd.DataFrame, pd.DataFrame]:
    data = scores.loc[
        scores["feature_set"].eq(config["feature_set"])
        & scores["tolerance_bps"].eq(float(config["tolerance_bps"]))
        & scores["model"].isin(config["models"])
        & scores["horizon"].isin(config["horizons"])
    ].copy()
    data["timestamp"] = pd.to_datetime(data["timestamp"], errors="raise")
    fold_rows: list[dict[str, object]] = []
    signal_frames: list[pd.DataFrame] = []
    for keys, fold in data.groupby(["scope", "model", "horizon", "test_year"], sort=True):
        scope, model, horizon, year = keys
        fold = fold.sort_values(["corridor", "timestamp"], kind="mergesort").copy()
        fold["week"] = fold["timestamp"].dt.to_period("W").astype(str)
        weekly_rate = fold.groupby(["corridor", "week"])["target"].mean()
        fold["matched_week_hit_rate"] = [
            float(weekly_rate.loc[(corridor, week)])
            for corridor, week in zip(fold["corridor"], fold["week"], strict=True)
        ]
        exposure = int(fold["corridor"].nunique())
        duration = max(float((fold["timestamp"].max() - fold["timestamp"].min()).days) / 7, 1 / 7) * exposure
        for policy in config["policies"]:
            candidate_parts: list[pd.DataFrame] = []
            for _, group in fold.groupby("corridor", sort=False):
                if policy == "adaptive_top40":
                    candidates = adaptive_candidates(
                        group,
                        share=float(config["adaptive_top_score_share"]),
                        lookback=int(config["adaptive_lookback_observations"]),
                        minimum_history=int(config["adaptive_minimum_history"]),
                    )
                else:
                    alpha = float(policy.removeprefix("conformal_alpha_"))
                    candidates = conformal_positive_candidates(
                        group,
                        horizon=int(horizon),
                        alpha=alpha,
                        lookback=int(config["calibration_lookback_observations"]),
                        minimum_history=int(config["minimum_calibration_observations"]),
                    )
                candidate_parts.append(candidates)
            candidates = pd.concat(candidate_parts, ignore_index=False)
            selected = pd.concat(
                [
                    _apply_policy(
                        group,
                        cooldown_days=int(config["cooldown_days"]),
                        weekly_cap=int(config["weekly_cap"]),
                    )
                    for _, group in candidates.groupby("corridor", sort=False)
                ],
                ignore_index=False,
            ) if len(candidates) else candidates.copy()
            count = len(selected)
            base_rate = float(fold["target"].mean())
            hit_rate = float(selected["target"].mean()) if count else np.nan
            identity = {
                "scope": scope,
                "model": model,
                "feature_set": policy,
                "horizon": int(horizon),
            }
            fold_rows.append(
                {
                    **identity,
                    "test_year": int(year),
                    "test_count": len(fold),
                    "test_hits": int(fold["target"].sum()),
                    "signal_count": count,
                    "signal_hits": int(selected["target"].sum()) if count else 0,
                    "lift": hit_rate / base_rate if count and base_rate > 0 else np.nan,
                    "matched_expected_hits": float(selected["matched_week_hit_rate"].sum()) if count else 0.0,
                    "regret_sum_bps": float(selected["regret_bps"].sum()) if count else 0.0,
                    "benefit_sum_bps": float(selected["benefit_bps"].sum()) if count else 0.0,
                    "duration_weeks": duration,
                }
            )
            if count:
                exported = selected[[
                    column for column in [
                        "timestamp", "corridor", "target", "regret_bps", "benefit_bps", "score",
                        "score_threshold", "matched_week_hit_rate", "conformal_p_positive", "conformal_p_negative",
                    ] if column in selected
                ]].copy()
                for key, value in identity.items():
                    exported[key] = value
                exported["test_year"] = int(year)
                signal_frames.append(exported)
    signals = pd.concat(signal_frames, ignore_index=True) if signal_frames else pd.DataFrame()
    return pd.DataFrame(fold_rows), signals


def run(
    *,
    config_path: Path | str = Path("configs/conformal_abstention_experiment.json"),
    artifact_dir: Path | str = Path("artifacts/next_hypotheses/conformal"),
) -> dict[str, object]:
    config = load_config(config_path)
    input_path = Path(config["scores_input"])
    folds, signals = evaluate(pd.read_csv(input_path), config)
    summary_frame = summarize(folds)
    output = Path(artifact_dir)
    output.mkdir(parents=True, exist_ok=True)
    folds.to_csv(output / "folds.csv", index=False)
    signals.to_csv(output / "signals.csv", index=False)
    summary_frame.to_csv(output / "summary.csv", index=False)
    meta = {
        "config_path": str(config_path),
        "config_sha256": hashlib.sha256(Path(config_path).read_bytes()).hexdigest(),
        "scores_sha256": hashlib.sha256(input_path.read_bytes()).hexdigest(),
        "fold_rows": len(folds),
        "signal_rows": len(signals),
        "warning": "Rolling conformal heuristic under non-stationarity; labels are delayed by the forecast horizon.",
    }
    (output / "run_meta.json").write_text(json.dumps(meta, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    return meta


def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--config", type=Path, default=Path("configs/conformal_abstention_experiment.json"))
    parser.add_argument("--artifact-dir", type=Path, default=Path("artifacts/next_hypotheses/conformal"))
    args = parser.parse_args(argv)
    print(json.dumps(run(config_path=args.config, artifact_dir=args.artifact_dir), ensure_ascii=False, indent=2))


if __name__ == "__main__":
    main()
